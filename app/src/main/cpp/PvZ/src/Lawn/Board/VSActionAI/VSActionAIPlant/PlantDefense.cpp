...
bool PlantAIPlanning::ShouldDeployWallnut(const VSGameState &state, int row) const {
    ...
    return (mowerlessEmergency || closest->eating || (hasIcebergControl && closest->positionX <= 720.0f)
               || (closest->positionX <= triggerDistance && lane.danger >= (decisive ? 80 : 105)))
...
std::optional<VSAction> PlantAIPlanning::TryCactusSpikeweedPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!hasCactusCard || CountPlantType(state, SeedType::SEED_CACTUS) == 0
        || PlantAIPlanning::FindReadyCard(state, SeedType::SEED_SPIKEWEED) == nullptr) {
        return std::nullopt;
    }
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        if (HasPlantTypeInRow(state, SeedType::SEED_SPIKEWEED, row)
            || FindClosestZombie(state, row) == nullptr

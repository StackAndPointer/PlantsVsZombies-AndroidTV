...
PlantDecisionResult PlantAI::TryOpeningOutputPhase(const VSGameState &state, const PlantDecisionContext &context) {
    if (context.openingNeedsFirepower) {
        if (context.hasSustainedOutputSeed) {
            if (std::optional<VSAction> action = TrySustainedOutputPlant(state, context.firepowerRow,
                    {.protectedSun = context.protectedSun, .allowLowCostCombat = true, .requirePreferredRow = true, .allowEmergencyTrade = true})) {
...
PlantDecisionResult PlantAI::TryTemplatePressurePhase(const VSGameState &state, const PlantDecisionContext &context) {
    const bool mustFundMainCarry = context.hasIncomeSeed && context.actualIncomePlantCount < context.openingIncomeTarget
        ...
        && context.danger.danger < 145 && (context.counterFirepower.canHold || context.weakestFirepower.closestDistance > 760);
    ...
    if (context.hasActiveZombie && !context.immediateCounterThreat
        && (context.largestFirepowerDeficit > 0 || context.counterLane.danger >= 95)) {
        if (std::optional<VSAction> action = TryWakeableMushroomOutput(state, context.firepowerRow, context.protectedSun)) {
            return {.handled = true, .action = action};
        }
    }
...
PlantDecisionResult PlantAI::TryEconomyConversionPhase(const VSGameState &state, const PlantDecisionContext &context) {
    if (context.highSunCombatPressure && !context.immediateCounterThreat) {
        if (std::optional<VSAction> action = TrySustainedOutputPlant(state, context.firepowerRow,
                {.protectedSun = context.protectedSun, .allowLowCostCombat = true})) {
            return {.handled = true, .action = action};
        }
    }
    if (context.hasActiveZombie && !context.immediateCounterThreat && context.hasSustainedOutputSeed
        && state.plantSun - context.protectedSun >= 200
        && (context.danger.danger >= 80 || context.largestFirepowerDeficit > 0)) {
        if (std::optional<VSAction> action = TrySustainedOutputPlant(state, context.firepowerRow,
                {.protectedSun = context.protectedSun, .allowLowCostCombat = true, .allowEmergencyTrade = true})) {
            return {.handled = true, .action = action};
        }
    }
    if (context.canStrikeZombieEconomy) {
        ...
        if (context.hasSustainedOutputSeed) {
            if (std::optional<VSAction> action = TrySustainedOutputPlant(state, context.zombieEconomyStrikeRow,
                    {.protectedSun = context.protectedSun})) {
                return {.handled = true, .action = action};
            }
        }
    }
    const bool daytimePadTemplate = !state.isNight && context.hasSunshroomFiller && !context.hasIncomeSeed;
    if (!state.isNight && context.hasSunshroomFiller && !context.immediateCounterThreat
        && (context.hasActiveZombie || daytimePadTemplate)
    ...
    const bool safeIncomeShortfall = context.incomePlantCount < context.openingIncomeTarget
        && context.danger.danger < 105 && !context.pressureOutrunsFirepower;
    if (context.hasIncomeSeed && context.incomePlantCount < context.openingIncomeTarget && context.danger.danger < 150
        && (!context.highSunCombatPressure || safeIncomeShortfall)) {
    ...
    const bool canExpandIncome = context.mayExpandIncomePastOpening
        && (context.danger.danger < 105 || (context.counterCombatPlants > 0 && context.danger.danger < 140))
        && (context.counterFirepower.canHold || context.weakestFirepower.closestDistance > 760);
    if (context.hasIncomeSeed && context.hasActiveZombie && context.incomePlantCount < context.incomeExpansionTarget
        && !context.immediateCounterThreat && canExpandIncome
        && (!context.highSunCombatPressure || (context.midGame && context.incomePlantCount < context.incomeExpansionTarget))
        && !context.outputTempoHasPriority) {
    ...
    if (context.hasIncomeSeed && context.incomePlantCount < context.incomeExpansionTarget
        && (!context.highSunCombatPressure || (context.midGame && context.incomePlantCount < context.incomeExpansionTarget))
        && !context.outputTempoHasPriority && context.mayExpandIncomePastOpening) {
        if (std::optional<VSAction> action = TryIncomePlant(state, buildRow, context.protectedSun)) {
            return {.handled = true, .action = action};
        }
    }
...
PlantDecisionResult PlantAI::TryLaneDefensePhase(const VSGameState &state, const PlantDecisionContext &context) {
    const bool hasRangedHarasser = HasZombieTypeInRow(state, context.danger.row, ZombieType::ZOMBIE_PEA_HEAD)
        || HasZombieTypeInRow(state, context.danger.row, ZombieType::ZOMBIE_SUNDAY_EDITION);
    ...
    if (context.danger.danger >= 105 && !yieldDangerLane) {
        if (!IsRangedOutputTradeUnfavorable(state, context.danger.row)
            && !HasPlantTypeInRow(state, SeedType::SEED_SNOWPEA, context.danger.row)) {
    ...
    if (context.hasActiveZombie && !ShouldYieldLaneToMower(state, buildRow)

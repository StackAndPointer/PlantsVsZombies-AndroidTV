...
std::optional<VSAction> PlantAIPlanning::TryTemplateSustainedOutput(const VSGameState &state, int preferredRow, int protectedSun,
    ...
    const int cost = card == nullptr ? std::numeric_limits<int>::max()
                                     : (policy.useEffectiveCost ? EffectivePlantPlayCost(state, *card) : card->cost);
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        if (HasPlantTypeInRow(state, policy.seed, row) || ShouldYieldLaneToMower(state, row)
            || (policy.requireFavorableRangedTrade && IsRangedOutputTradeUnfavorable(state, row))) {
            continue;
        }
        ...
        score += closest == nullptr ? policy.noZombieScore
                                    : (closest->positionX > policy.distantZombieThreshold ? policy.distantZombieScore
                                                                                           : policy.closeZombieScore);

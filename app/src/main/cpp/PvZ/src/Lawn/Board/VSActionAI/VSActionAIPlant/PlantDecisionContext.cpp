...
PlantDecisionContext PlantAIPlanning::BuildDecisionContext(const VSGameState &state) const {
    ...
    for (int row = 0; row < state.rows; ++row) {
        ...
        if (firepower.deficit > largestFirepowerDeficit
            || (firepower.deficit == largestFirepowerDeficit && !firepower.canHold && row != context.counterRow)) {
    ...
    const bool hasGargantuar = HasZombieTypeInRow(state, context.counterRow, ZombieType::ZOMBIE_GARGANTUAR)
        || HasZombieTypeInRow(state, context.counterRow, ZombieType::ZOMBIE_GIGA_GARGANTUAR);
    ...
    const bool earlySingleBucket = state.boardTick < 32000 && counterZombieCount == 1
        && HasZombieTypeInRow(state, context.counterRow, ZombieType::ZOMBIE_PAIL) && context.counterCombatPlants == 0 && context.counterLane.plantCount <= 2
        && context.counterClosest != nullptr && context.counterClosest->positionX < 700.0f && !context.counterFirepower.canHold;
    ...
    const int lateIncomeRecoveryTarget = !state.isSuddenDeath && context.midGame && context.actualIncomePlantCount < context.openingIncomeTarget
        ? context.openingIncomeTarget
        : 0;
    const int economyPressureIncomeTarget = EconomyPressureIncomeTarget(state);
    context.incomeExpansionTarget = state.isSuddenDeath ? 0
        : std::max(economyPressureIncomeTarget, lateIncomeRecoveryTarget);
    context.pressureOutrunsFirepower = context.hasActiveZombie && (unholdableZombieRows > 0
    ...
    const bool mustHoldCounterReserve = context.areaCounterReserve > 0 && state.plantSun >= context.areaCounterReserve
        && (HasReadyZombieBreakthroughCard(state) || ((heavyZombieDeck || swarmZombieDeck) && context.hasActiveZombie));
    ...
    const int economyStrikeIncomeFloor = context.economyZombieDeck
        ? std::max(2, context.minimumIncomeBeforeOutput - 1)
        : context.minimumIncomeBeforeOutput;
    context.canStrikeZombieEconomy = (state.isSuddenDeath || context.incomePlantCount >= economyStrikeIncomeFloor) && hasEconomyPressurePlan
        && CountZombieEconomy(state) > 0 && context.danger.danger < 150 && !context.immediateCounterThreat;
    ...
    context.desiredOutputCount = state.isSuddenDeath
        ? context.maximumOutputCount
        : std::min(context.maximumOutputCount, std::max(1, (context.incomePlantCount + 1) / 2));
    ...
    if (!state.isSuddenDeath && hasSporeCarry) {
        const int sporeLayerTarget = state.plantSun >= 500 ? context.maximumOutputCount
    ...
    context.needsSustainedOutput = context.hasSustainedOutputSeed
        && (context.sustainedOutputCount < context.desiredOutputCount || (context.hasActiveZombie && context.largestFirepowerDeficit > 0));
    const int highSunCombatTarget = std::min(context.maximumOutputCount, state.rows + (state.plantSun >= 350 ? 2 : 1));
    context.highSunCombatPressure = !state.isSuddenDeath && state.plantSun >= 125
        && context.incomePlantCount >= context.minimumIncomeBeforeOutput && (hasCombatSeed || context.hasSustainedOutputSeed)
    ...
    const bool deckOpeningPressure = deckNeedsEarlyFirepower && !context.hasActiveZombie
        && context.incomePlantCount >= context.minimumIncomeBeforeOutput && state.plantSun >= 100;
    context.openingNeedsFirepower = deckOpeningPressure || (context.hasActiveZombie && (context.counterFirepower.deficit > 0 || !context.counterFirepower.canHold
        || (context.counterClosest != nullptr && context.counterClosest->positionX < 720.0f && context.counterCombatPlants == 0)));
    context.outputTempoHasPriority = context.incomePlantCount >= context.openingIncomeTarget && context.needsSustainedOutput
        && context.readySustainedOutput && (context.pressureOutrunsFirepower || (context.midGame && context.incomePlantCount >= context.incomeExpansionTarget));
    context.mayExpandIncomePastOpening = context.incomePlantCount < context.openingIncomeTarget || !context.midGame
        || (!context.pressureOutrunsFirepower && (!context.needsSustainedOutput || !context.readySustainedOutput
            || context.incomePlantCount < context.incomeExpansionTarget));
    return context;
}

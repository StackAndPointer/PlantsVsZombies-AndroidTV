...
std::optional<VSAction> PlantAIPlanning::TryMelonScaredySupport(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        if (target.col < 0 || target.row < 0 || PlantAIPlanning::ShouldYieldLaneToMower(state, row)
            || !IsPlantPlacementSafe(state, SeedType::SEED_SCAREDYSHROOM, target)) {
            continue;
        }
...
std::optional<VSAction> PlantAIPlanning::TryScaredyMelonSupport(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const int targetCount = std::min(state.rows * 2, state.rows + CountPlantIncome(state) / 2);
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        if (PlantAIPlanning::ShouldYieldLaneToMower(state, row)
            || (closest != nullptr && closest->positionX < 620.0f)) {
            continue;
        }
    ...
    return bestCard == nullptr ? std::nullopt
                               : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *bestCard, bestTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryScaredyPuffDoomPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const VSCardState *scaredy = PlantAIPlanning::FindReadyCard(state, SeedType::SEED_SCAREDYSHROOM);
    const int scaredyCost = scaredy == nullptr ? std::numeric_limits<int>::max() : PlantAIPlanning::EffectivePlantPlayCost(state, *scaredy);
    const int scaredyTarget = std::min(state.rows, std::max(3, CountPlantIncome(state) - 1));
    ...
    if (puff == nullptr || puffCost == std::numeric_limits<int>::max() || state.plantSun - puffCost < protectedSun
        || CountPlantType(state, SeedType::SEED_SCAREDYSHROOM) == 0) {
        return std::nullopt;
    }
    ...
    return bestTarget.col < 0 || bestTarget.row < 0
        ? std::nullopt
        : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *puff, bestTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryPeaPuffPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const bool earlyPuffResponse = CountActiveZombies(state) > 0 && CountPlantIncome(state) >= 2;
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        const VSZombieState *closest = FindClosestZombie(state, row);
        if (closest == nullptr || closest->positionX > 720.0f
            || HasPlantTypeInRow(state, SeedType::SEED_PUFFSHROOM, row)
            || PlantAIPlanning::ShouldYieldLaneToMower(state, row)) {
            continue;
        }
        const VSGridPosition target = FindPlantCellInExactRow(state, row, 2, 3);
        ...
        const PlantLaneFirepower firepower = AssessPlantLaneFirepower(state, row);
        const bool hasPeaFamilyInRow = HasPlantTypeInRow(state, SeedType::SEED_PEASHOOTER, row)
            || HasPlantTypeInRow(state, SeedType::SEED_REPEATER, row)
            || HasPlantTypeInRow(state, SeedType::SEED_THREEPEATER, row);
...
std::optional<VSAction> PlantAIPlanning::TrySporePuffPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const VSCardState *puff = PlantAIPlanning::FindReadyCard(state, SeedType::SEED_PUFFSHROOM);
    if (!hasSporeCarry || puff == nullptr || CountPlantIncome(state) < 3) {
        return std::nullopt;
    }
        ...
        if (plant.seedType == static_cast<std::uint16_t>(SeedType::SEED_MAGNETSHROOM)) {
            ...
            for (const VSZombieState &zombie : state.zombies) {
                if (!zombie.dead && zombie.row == plant.position.row
                    && (zombie.zombieType == static_cast<std::uint16_t>(ZombieType::ZOMBIE_PAIL)
                        || zombie.zombieType == static_cast<std::uint16_t>(ZombieType::ZOMBIE_DOOR)
                        || zombie.zombieType == static_cast<std::uint16_t>(ZombieType::ZOMBIE_FOOTBALL)
                        || zombie.zombieType == static_cast<std::uint16_t>(ZombieType::ZOMBIE_TRASHCAN)

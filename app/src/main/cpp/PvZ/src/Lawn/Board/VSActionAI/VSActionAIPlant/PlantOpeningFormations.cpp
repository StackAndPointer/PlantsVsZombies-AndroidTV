...
std::optional<VSAction> PlantAIPlanning::TryBoomerangControlPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!boomerangControlTemplate || CountPlantIncome(state) < 4 || CountZombieEconomy(state) == 0) {
        return std::nullopt;
    }
    ...
    const int firingLineTarget = std::min(state.rows, std::max(1, CountPlantIncome(state) - 3));
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        if (HasPlantTypeInRow(state, SeedType::SEED_BLOOMERANG, row) || PlantAIPlanning::ShouldYieldLaneToMower(state, row)
...
std::optional<VSAction> PlantAIPlanning::TryThreepeaterPuffFormation(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!threepeaterPuffTemplate || CountPlantIncome(state) < 4 || CountZombieEconomy(state) == 0) {
        return std::nullopt;
    }
    ...
    const int formationTarget = std::min(3, std::max(1, CountPlantIncome(state) - 3));
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        if (row == 0 || row == state.rows - 1 || HasPlantTypeInRow(state, SeedType::SEED_THREEPEATER, row)
    ...
    return bestTarget.col < 0 || bestTarget.row < 0
        ? std::nullopt
        : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *threepeater, bestTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TrySnowpeaPuffMagnetPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (CountPlantIncome(state) >= 1 && CountPlantIncome(state) < 3 && puff != nullptr
    ...
    if (puff != nullptr && CountPlantType(state, SeedType::SEED_PUFFSHROOM) < puffSupportLimit
        && (state.isNight || coffee != nullptr) && state.plantSun - puffCost >= protectedSun) {
    ...
    const int snowpeaTarget = std::min(state.rows, std::max(1, CountPlantIncome(state) - 2));
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        if (HasPlantTypeInRow(state, SeedType::SEED_SNOWPEA, row) || PlantAIPlanning::ShouldYieldLaneToMower(state, row)
            || IsRangedOutputTradeUnfavorable(state, row)) {
            continue;
        }
    ...
    return bestTarget.col < 0 || bestTarget.row < 0
        ? std::nullopt
        : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *snowpea, bestTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryPeaPuffTempoOpening(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!peaPuffTemplate || state.isSuddenDeath || CountPlantIncome(state) < 1 || CountZombieEconomy(state) == 0) {
        return std::nullopt;
    }
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        ...
        const bool hasAwakePuff = std::any_of(state.plants.begin(), state.plants.end(), [row](const VSPlantState &plant) {
            return !IsDeadOrOutside(plant) && plant.position.row == row
                && plant.seedType == static_cast<std::uint16_t>(SeedType::SEED_PUFFSHROOM) && !plant.asleep;
        });
        if (!hasAwakePuff || HasPlantTypeInRow(state, SeedType::SEED_PEASHOOTER, row)
            || PlantAIPlanning::ShouldYieldLaneToMower(state, row) || IsRangedOutputTradeUnfavorable(state, row)) {
            continue;
        }
        ...
        const PlantLaneFirepower firepower = AssessPlantLaneFirepower(state, row);
        const int score = OpeningScore(row) + firepower.deficit * 12 + (firepower.canHold ? 0 : 100)
            + StrategyBonus(state, VSSide::Plants, SeedType::SEED_PEASHOOTER, row);
        if (bestTarget.col < 0 || score > bestScore) {
            bestTarget = target;
            bestScore = score;
        }
...
std::optional<VSAction> PlantAIPlanning::TrySporePuffTempoPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const int incomeCount = CountPlantIncome(state);
    ...
    if (puff != nullptr && CountPlantType(state, SeedType::SEED_PUFFSHROOM) < puffTarget
        && (state.isNight || coffee != nullptr) && state.plantSun - puffCost >= protectedSun) {
        VSGridPosition bestPuffTarget{};
        int bestPuffScore = std::numeric_limits<int>::min();
        for (int offset = 0; offset < state.rows; ++offset) {
    ...
    if (spore == nullptr || sporeCost == std::numeric_limits<int>::max() || state.plantSun - sporeCost < protectedSun
        || CountPlantType(state, SeedType::SEED_SPORESHROOM) >= sporeTarget) {
        return std::nullopt;
    }
    ...
    return bestSporeTarget.col < 0 ? std::nullopt
                                   : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *spore, bestSporeTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryPeaCabbageTorchTempo(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!peaCabbageTorchTemplate || CountPlantIncome(state) < 3 || CountZombieEconomy(state) == 0) {
        return std::nullopt;
    }
...
std::optional<VSAction> PlantAIPlanning::TrySnowpeaBonkFormation(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!snowpeaBonkTemplate || state.isSuddenDeath || CountPlantIncome(state) < 3 || CountZombieEconomy(state) == 0) {
        return std::nullopt;
    }
    ...
    const auto IsSafeFormationRow = [&](int row) {
        const VSZombieState *closest = FindClosestZombie(state, row);
        return !PlantAIPlanning::ShouldYieldLaneToMower(state, row)
            && (closest == nullptr || (!closest->eating && closest->positionX > 660.0f));
    };
    ...
    if (wallnutCount > 0 && bonkCount == 0 && IsAffordable(bonk)) {
        ...
        for (int offset = 0; offset < state.rows; ++offset) {
            const int row = (preferredRow + offset) % state.rows;
            if (!HasPlantTypeInRow(state, SeedType::SEED_WALLNUT, row)
                || HasPlantTypeInRow(state, SeedType::SEED_BONK_CHOY, row) || !IsSafeFormationRow(row)) {
                continue;
            }
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        if (HasPlantTypeInRow(state, SeedType::SEED_SNOWPEA, row) || !IsSafeFormationRow(row)
            || IsRangedOutputTradeUnfavorable(state, row)) {
            continue;
        }
        const VSGridPosition target = PlantAIPlanning::FindSustainedOutputCell(state, SeedType::SEED_SNOWPEA, row);

...
ZombieDecisionContext ZombieAIPlanning::BuildDecisionContext(const VSGameState &state) const {
    ...
    context.targetDefenseEmergency = context.targetMarkersOnBoard > 0
        && historicalTargetLosses + context.zeroHealthTargetMarkers >= 2;
    ...
    context.graveDefenseUrgent = context.targetDefenseEmergency || context.graveDefenseScore >= 50 || context.graveStraightThreat >= 55
        || context.graveLobbedThreat >= 70 || context.graveScreenDeficit >= 55 || proactiveGraveScreen;
    context.graveDefenseReinforcement = context.graveDefenseUrgent
        && (context.targetDefenseEmergency || !context.hasGraveGuard || context.graveScreenDeficit >= 55);
    context.heavyZombieReserve = HeavyZombieReserve(state);
    context.bankForHeavy = context.heavyZombieReserve >= 100
        && context.economyCount >= context.tempo.HeavyBankEconomyThreshold(state.rows, context.heavyEconomyThreshold)
        && context.tempo.HasAttackCommitPressure(context.activePressureRows, 2, state.rows)
        && CountLivePlants(state) >= state.rows && context.graveDefenseScore < 100;
    ...
    context.openingPressureEconomyFloor = (impPailSundayTemplate || zamboniPoleOpeningTemplate)
        ? context.tempo.OpeningEconomyFloor(std::min(3, state.rows))
        : context.minimumOpeningEconomy;
    context.firstGraveProbe = context.armoredNormalRushTemplate && context.actualEconomyCount == 1 && context.activePressureRows == 0
        && context.hasReadyFrontlineProbe && mLastPressureEconomyCount < context.actualEconomyCount;
    context.openingPressureEconomyCeiling = context.tempo.OpeningEconomyCeiling(state.rows + 1);
    context.openingPressureCadence = context.economyCount >= context.openingPressureEconomyFloor
        && context.economyCount <= context.openingPressureEconomyCeiling && context.activePressureRows < context.desiredOpeningRows
    ...
    context.forceOpeningPressure = context.firstGraveProbe || (context.hasReadyFrontlineProbe
        && (context.openingPressureCadence || context.enhancedPressureRecovery));
    context.preservePressureDuringRepair = context.economyCount >= context.minimumOpeningEconomy
        && context.economyDeficit <= context.tempo.PressureRepairDeficitTolerance()
    ...
    context.preserveSurvivingFront = context.criticalTargetRow >= 0
        || (context.economyCount >= state.rows && context.activePressureRows == 1 && context.survivingFrontValue >= 90);
    ...
    context.economyRepairIsUrgent = context.economyCount < context.minimumOpeningEconomy
        || context.economyDeficit >= context.tempo.EconomyRepairDeficitThreshold()
        || (context.tempo.IsEnhanced() && context.economyDeficit > 0 && context.activePressureRows >= std::min(2, state.rows));
    context.hasReadyTemplateCommit = HasReadyZombieTemplateCommit(state, context.templateProfile, context.tempo,

    ...
    for (int rowOffset = 0; rowOffset < state.rows; ++rowOffset) {
        ...
        for (int column = 5; column > frontOutputColumn; --column) {
            ...
            if (!IsPlantableVSTile(state, target) || HasPlantAt(state, target) || HasGridItemAt(state, target)
                || !IsPlantPlacementSafe(state, SeedType::SEED_POTATOMINE, target)) {
                continue;
            }
...
std::optional<VSAction> PlantAIPlanning::TrySnowpeaBonkPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (snowpea != nullptr && state.plantSun - snowpea->cost >= protectedSun) {
        ...
        for (int offset = 0; offset < state.rows; ++offset) {
            ...
            const VSZombieState *closest = FindClosestZombie(state, row);
            if (closest == nullptr || HasPlantTypeInRow(state, SeedType::SEED_SNOWPEA, row)
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        const VSZombieState *closest = FindClosestZombie(state, row);
        if (closest == nullptr || HasPlantTypeInRow(state, SeedType::SEED_BONK_CHOY, row)
            || closest->positionX > 660.0f || PlantAIPlanning::ShouldYieldLaneToMower(state, row)) {
            continue;
        }
        const PlantLaneFirepower firepower = AssessPlantLaneFirepower(state, row);
...
std::optional<VSAction> PlantAIPlanning::TryStarfruitChomperPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    if (!hasStarfruitDeck || chomper == nullptr || CountPlantType(state, SeedType::SEED_STARFRUIT) == 0
        || state.plantSun - chomper->cost < protectedSun) {
        return std::nullopt;
    }
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        const VSZombieState *closest = FindClosestZombie(state, row);
        if (closest == nullptr || HasPlantTypeInRow(state, SeedType::SEED_CHOMPER, row)
            || closest->positionX > 620.0f || PlantAIPlanning::ShouldYieldLaneToMower(state, row)) {
            continue;
        }
        const PlantLaneFirepower firepower = AssessPlantLaneFirepower(state, row);
...
std::optional<VSAction> PlantAIPlanning::TryKernelCeleryPressure(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const bool hasRearCore = CountPlantType(state, SeedType::SEED_KERNELPULT) > 0
        || CountPlantType(state, SeedType::SEED_REPEATER) > 0;
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        const VSZombieState *closest = FindClosestZombie(state, row);
        if (closest == nullptr || closest->mindControlled || closest->positionX > 660.0f
        ...
        if (target.col < 0 || target.row < 0 || PlantAIPlanning::ShouldYieldLaneToMower(state, row)
            || !IsPlantPlacementSafe(state, SeedType::SEED_CELERY_STALKER, target)) {
            continue;
        }
    ...
    return bestTarget.col < 0 || bestTarget.row < 0 ? std::nullopt
...
std::optional<VSAction> PlantAIPlanning::TryMagnetShroom(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    for (int offset = 0; offset < state.rows; ++offset) {
        const int row = (preferredRow + offset) % state.rows;
        if (HasPlantTypeInRow(state, SeedType::SEED_MAGNETSHROOM, row)
            || PlantAIPlanning::ShouldYieldLaneToMower(state, row)) {
            continue;
        }
        int metalCount = 0;
    ...
    return bestTarget.col < 0 || bestTarget.row < 0
        ? std::nullopt
        : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *magnet, bestTarget, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryHypnoshroom(const VSGameState &state, int preferredRow, int protectedSun) {
    ...
    const auto HasActiveSeed = [&state](SeedType seed) {
        return std::any_of(state.seedBanks[0].begin(), state.seedBanks[0].end(), [seed](const VSCardState &candidate) {
            return candidate.active && !candidate.matchRestricted && candidate.seedType == static_cast<std::uint16_t>(seed);
        });
    };
    const bool boomerangControlTemplate = HasActiveSeed(SeedType::SEED_BLOOMERANG)
        && HasActiveSeed(SeedType::SEED_DOOMSHROOM);
    ...
    int bestScore = std::numeric_limits<int>::min();
    for (const VSZombieState &zombie : state.zombies) {
        if (zombie.dead || zombie.mindControlled || !IsHypnoshroomTarget(zombie) || zombie.row < 0 || zombie.row >= state.rows
            || IsMowerInMotion(state, zombie.row) || zombie.positionX > 760.0f || HasMindControlledZombieInRow(state, zombie.row)) {
            continue;
        }

        int followers = 0;
        for (const VSZombieState &other : state.zombies) {
            if (!other.dead && !other.mindControlled && other.row == zombie.row && other.positionX > zombie.positionX + 45.0f
                && other.positionX < zombie.positionX + 500.0f) {
                ++followers;
            }
        }
        ...
        if (boomerangControlTemplate && !controlLineReady && !zombie.eating && zombie.positionX > 620.0f
            && PlantAIPlanning::ZombieEffectiveHealth(zombie) < 1200 && followers < 2) {
            continue;
        }
    ...
    return bestZombie == nullptr ? std::nullopt
...
std::optional<VSAction> PlantAIPlanning::TryWakeSleepingDoomshroom(const VSGameState &state) {
    ...
    for (const VSPlantState &plant : state.plants) {
        if (!IsDeadOrOutside(plant) && !IsMowerInMotion(state, plant.position.row) && plant.asleep
            && plant.seedType == static_cast<std::uint16_t>(SeedType::SEED_DOOMSHROOM)) {
            doomshroom = &plant;
            break;
        }
    }
    return doomshroom == nullptr ? std::nullopt
        : std::optional<VSAction>(MakePlayAction(VSSide::Plants, *coffee, doomshroom->position, state.boardTick));
}
...
std::optional<VSAction> PlantAIPlanning::TryIcebergLettuce(const VSGameState &state, int row, int protectedSun, bool forceEmergencyControl) {
    ...
    return target.col < 0 || target.row < 0
        ? std::nullopt
...
std::optional<VSAction> PlantAIPlanning::TryTorchwoodSupport(const VSGameState &state, int protectedSun) {
    ...
    for (int row = 0; row < state.rows; ++row) {
        ...
        const int score = peaFamilyCount * 180 + PlantEconomyValueInRow(state, row) / 2
            + StrategyBonus(state, VSSide::Plants, SeedType::SEED_TORCHWOOD, row);
